...
class LastAnalysisFragment : Fragment() {
    ...
    private fun loadLastAnalysis() {
        lifecycleScope.launch(Dispatchers.IO) {
            // Charger la derni̬re fiche consult̩e (analyse, r̩-analyse ou consultation historique)
            var entry = analysisHistoryManager.getLastViewedCard()
            val history = analysisHistoryManager.getAnalysisHistory()
            // V̩rifier si la derni̬re fiche consult̩e existe toujours dans l'historique
            if (entry != null && history.none { it.imageUri == entry!!.imageUri }) {
                // Si elle n'existe plus, trouver la premi̬re fiche non-tutorielle dans l'historique
                entry = history.firstOrNull { !it.isTutorial }
                if (entry != null) {
                    analysisHistoryManager.saveLastViewedCard(entry)
                } else {
                    // Si aucune fiche n'existe, utiliser une fiche exemple par d̩faut
                    entry = history.firstOrNull { it.isTutorial }
                    if (entry == null) {
                        // Si aucune fiche tutorielle n'existe, on pourrait en cr̩er une par d̩faut ici si n̩cessaire
                        // Pour l'instant, on laisse vide si aucune fiche tutorielle n'est pr̩sente
                    } else {
                        analysisHistoryManager.saveLastViewedCard(entry)
                    }
                }
            ...
            withContext(Dispatchers.Main) {
                if (entry != null) {
                    ...
                    // Utiliser Coil pour charger l'image de mani̬re fiable
                    ...
                    // Afficher l'ic̫ne danger sur la photo si l'esp̬ce est dangereuse
                        ...
                        entry.representativeColorHex?.let { colorHex ->
                            if (colorHex.startsWith("#") && colorHex.length == 7) {
                                try {
                                    ...
                                    lastAnalysisTypeBadge.setBackgroundResource(R.drawable.badge_nc) // Couleur par d̩faut en cas d'erreur
                                }
                            } else {
                                lastAnalysisTypeBadge.setBackgroundResource(R.drawable.badge_nc) // Couleur par d̩faut si le format est incorrect
                            }
                        } ?: run { lastAnalysisTypeBadge.setBackgroundResource(R.drawable.badge_nc) } // Couleur par d̩faut si la couleur est nulle
                    }
                    ...
                    // Si aucune fiche n'est trouv̩e, chercher une fiche tutorielle par d̩faut
